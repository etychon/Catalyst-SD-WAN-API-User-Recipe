...
- **Scripts** live in `samples/scripts/` — thin CLIs; configuration via `.env` (see `samples/.env.example`). Use `collect_dashboard_snapshot.py` for a multi-endpoint export aligned with [docs/dashboard-architecture.md](docs/dashboard-architecture.md).
...
| `docs/recipes/cellular-signal-thresholds.md` | `samples/scripts/cellular_thresholds.py` |
