...
| Hosted Edge Services (IOx) | `POST /statistics/apphosting/page`, `GET /statistics/apphosting/doccount`, `POST /statistics/apphostinginterface/page`, `GET /device` | Fleet IOx app health, version, resource usage via statistics plane; join inventory for reachability. Deploy via config-group recipes. See [hosted-edge-services-iox.md](recipes/hosted-edge-services-iox.md). |
