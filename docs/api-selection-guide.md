...
| UX 2.0 CSV onboard | `GET /device`, `POST /v1/config-group/{id}/device/associate`, `PUT /v1/config-group/{id}/device/variables`, `POST .../device/deploy`, `GET /device/action/status/{processId}` | Discover reachable unassigned devices; CSV-driven associate + variables + deploy + task poll. See [config-group-csv-onboard-deploy.md](recipes/config-group-csv-onboard-deploy.md). |
| Hosted Edge Services (IOx) | `POST /statistics/apphosting/page`, `POST /statistics/apphosting/doccount` (filtered; same query as page), optional `GET /statistics/apphosting/doccount` (unfiltered), `POST /statistics/apphostinginterface/page`, `GET /device` | Fleet IOx app health, version, resource usage via statistics plane; join inventory for reachability. Deploy via config-group recipes. See [hosted-edge-services-iox.md](recipes/hosted-edge-services-iox.md). |
